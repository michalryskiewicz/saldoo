 ...
 *
 * Costs entered before the question was asked answer it from the priority they were given, which
 * is the field this one replaced. `LOW` is the only value a person had to choose deliberately —
 * `MEDIUM` was the form's default — so it is the only one that changes anything, and every
 * emergency fund on any device reads exactly as it did before.
 */
