...
export const useGoalRecords = () => {
  ...
  // window the *contributions* fall in, an account that had put nothing aside asked for no window
  // at all, and every target on it printed its złoty figure under a euro sign — on both screens
  // that read this, since they read it precisely so they cannot disagree.
  ...
  const { data: exchangeRates } = useListExchangeRatesQuery(
    ...
    { skip: !goals.length && !contributions.length }
  );
